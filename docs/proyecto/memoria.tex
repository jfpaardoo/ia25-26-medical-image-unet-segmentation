...
Para asegurar la reproducibilidad exacta del experimento y evitar inconsistencias en las versiones de los datos, se ha desarrollado un script automatizado de adquisición (\texttt{scripts/download\_drive.py}). Este módulo utiliza la librería \texttt{kagglehub} para conectarse remotamente a un repositorio público y descargar de forma íntegra la versión del dataset DRIVE 2004.

Además de la descarga, el script realiza una labor fundamental de limpieza estructural. Las máscaras de segmentación originales se proporcionan en formato \texttt{.gif}, el cual suele presentar paletas de colores o metadatos de múltiples fotogramas que provocan errores de lectura en librerías como Keras. Para solucionar esto de raíz, el script lee cada máscara, la binariza estrictamente a valores absolutos (0 y 255) para obtener un \textit{ground truth} perfectamente limpio, y la guarda en formato \texttt{.png}. Finalmente, reorganiza los archivos en una estructura de carpetas unificada (\texttt{data/raw/training} y \texttt{data/raw/test}), preparando el terreno para el entrenamiento.
...
Para ensamblar nuevamente estos parches ordenados y formar la imagen médica completa, se emplea una técnica de promediado o difuminado (\textit{blending}). Al realizar el recorrido con la ventana deslizante, se fuerza un solapamiento espacial intencionado. Los píxeles situados en las zonas de intersección reciben, por tanto, múltiples predicciones probabilísticas (una por cada parche que se superpone en esa coordenada). El algoritmo acumula internamente estos valores y calcula la media aritmética final de cada píxel, logrando así difuminar cualquier corte brusco y garantizando una transición visualmente continua y coherente a lo largo de toda la red capilar.
...
El riguroso proceso de certificación predictiva del modelo se materializó al reestructurar de manera integral el script de pruebas \texttt{scripts/evaluate.py}. Este entorno analítico asegura el cálculo imperturbable del DICE score comparando las inferencias de la U-Net contra las delineaciones anatómicas provenientes de sendos anotadores clínicos humanos. Dado que \texttt{scripts/download\_drive.py} normaliza las máscaras a \texttt{.png} y las reubica en \texttt{masks\_expert1/} y \texttt{masks\_expert2/}, el evaluador calcula el DICE buscando directamente \texttt{\{id\}\_test.png} en dichas carpetas. El script calcula tres resultados importantes: los promedios individuales obtenidos contra el primer y el segundo oftalmólogo, así como la media ponderada global de la muestra. Este grado de rigor evaluativo certifica una alineación plena con el protocolo de investigación original postulado por los creadores clínicos del repositorio DRIVE en la universidad médica correspondiente.

\section{Resultados y Conclusiones}

\subsection{Resultados Cuantitativos}
Una vez finalizado el entrenamiento y la validación cruzada (5-Fold), el modelo fue evaluado de forma automatizada sobre el conjunto de prueba (imágenes de la carpeta \texttt{artifacts/predictions/}). Los resultados obtenidos superan satisfactoriamente los requisitos establecidos para el proyecto.
