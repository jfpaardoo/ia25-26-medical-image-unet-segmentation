...
Para asegurar una total reproducibilidad del experimento en cualquier entorno sin requerir intervención manual, se programó el script \texttt{scripts/download\_drive.py}, encargado de automatizar la descarga del conjunto DRIVE 2004 mediante la API de \texttt{kagglehub}. Dicho script ha sido refactorizado y acoplado específicamente a la topología de directorios de este dataset, optimizando las operaciones de disco. Itera directamente sobre los archivos \texttt{.tif} y realiza una binarización matemática directa de las intrincadas máscaras originales (formato \texttt{.gif}) a matrices estrictas de formato \texttt{.png} (valores 0 y 255). Tras la descarga, la función \texttt{discover\_drive\_samples()} en \texttt{src/data/dataset.py} se encarga de emparejar lógicamente cada imagen radiográfica con su máscara de experto, encapsulando las rutas de acceso en estructuras inmutables de tipo \texttt{SegmentationSample}.
